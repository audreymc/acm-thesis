...
class BasePredictor(ABC):
    def __init__(
        ...
        calib_frac=None,
        coverage=0.9, # coverage proportion
        horizon=1, # length of the forecast horizon
        ...
        **kwargs,
        ...
        if calib_residuals is not None:
            ...
            if calib_data is not None:
                self.forecast(calib_data, train_data) # calibration data, training data
    ...
    def forecast( # time_series is the new data to forecast on, and time_series_prev is the data used to train the model
        ...
        for i in tqdm.trange(len(time_series), desc="Forecasting", disable=not self.verbose):
            ...
            yhat_t, _ = self.model.forecast(y_t.index, TimeSeries.from_pd(time_series_prev)) # What to do here?
